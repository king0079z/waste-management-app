...
class MapManager {
    constructor() {
        ...
        // Scalability: when true, load bins via API bbox (getBinsInView) instead of full getBins()
        this.useBboxForBins = typeof window !== 'undefined' && !!window.__WASTE_USE_BBOX_BINS__;
    }
    ...
    updateDriverPosition(position) {
        ...
        const now = Date.now();
        if (now - this.lastDriverLocationSendTime >= 2500) {
    ...
    createDriverPopup(driver, location, todayCollections, isCurrentDriver = false, statusText = 'Active') {
        ...
        const activeRoutes = routes.filter(r => r.driverId === driver.id && r.status !== 'completed' && r.status !== 'cancelled').length;
    ...
    loadBinsOnMap(force) {
        ...
        const proceedWithBins = (bins) => this._addBinsToMap(bins, binsWithOpenPopups);
        if (this.useBboxForBins && typeof dataManager.getBinsInView === 'function' && this.map.getBounds) {
            const b = this.map.getBounds();
            const bbox = [b.getWest(), b.getSouth(), b.getEast(), b.getNorth()];
            dataManager.getBinsInView(bbox, { limit: 3000 }).then(({ bins }) => {
                console.log(`📦 Loading ${bins.length} bins on map (bbox API)...`);
                proceedWithBins(bins);
            }).catch(() => {
                proceedWithBins(dataManager.getBins());
            });
            return;
        }
    ...
    _addBinsToMap(bins, binsWithOpenPopups) {
        ...
        const addSensorBinsAfterRefresh = async () => {
            ...
            const refreshOne = (binId) => (findy.refreshBinSensor(binId) || Promise.resolve()).catch(() => {});
    ...
    startMapUpdates() {
        ...
        // When using bbox API, refetch bins on pan/zoom (debounced)
        if (this.useBboxForBins && this.map && !this._bboxMoveEndBound) {
            let bboxDebounce = null;
            this._bboxMoveEndBound = () => {
                if (bboxDebounce) clearTimeout(bboxDebounce);
                bboxDebounce = setTimeout(() => { this.loadBinsOnMap(); }, 400);
            };
            this.map.on('moveend', this._bboxMoveEndBound);
        }
