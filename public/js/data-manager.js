...
class DataManager {
    ...
    initializeDefaultData() {
        // Check if we have a large generated dataset
        const generatedBins = localStorage.getItem('generated_bins');
        const generatedDrivers = localStorage.getItem('generated_drivers');
    ...
    /**
     * API-first: fetch bins in a map bounding box (no full load). Use for 1M+ bins / map scalability.
     * @param {Array<number>} bbox - [minLng, minLat, maxLng, maxLat]
     * @param {{ limit?: number, offset?: number }} opts
     * @returns {Promise<{ bins: Array, total: number }>}
     */
    async getBinsInView(bbox, opts = {}) {
        const base = (typeof syncManager !== 'undefined' && syncManager.baseUrl) ? syncManager.baseUrl.replace(/\/$/, '') : (typeof window !== 'undefined' ? window.location.origin : '');
        const limit = Math.min(opts.limit || 2000, 10000);
        const offset = Math.max(0, opts.offset || 0);
        const params = new URLSearchParams({ limit: String(limit), offset: String(offset) });
        if (bbox && bbox.length >= 4) params.set('bbox', bbox.join(','));
        try {
            const res = await fetch(`${base}/api/bins?${params}`, { method: 'GET', headers: { 'Content-Type': 'application/json' } });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            const json = await res.json();
            if (!json.success || !Array.isArray(json.bins)) return { bins: [], total: 0 };
            const bins = json.bins.map(b => this.normalizeBin(b));
            return { bins, total: json.total != null ? json.total : bins.length };
        } catch (e) {
            console.warn('getBinsInView failed, falling back to local getBins:', e.message);
            const all = this.getBins();
            let list = all;
            if (bbox && bbox.length >= 4) {
                const [minLng, minLat, maxLng, maxLat] = bbox;
                list = all.filter(b => b.lat >= minLat && b.lat <= maxLat && b.lng >= minLng && b.lng <= maxLng);
            }
            return { bins: list.slice(offset, offset + limit), total: list.length };
        }
    }

    /**
     * Update a single bin via API (no full-sync). Prefer for scalable updates.
     */
    async updateBinViaAPI(binId, updates) {
        const base = (typeof syncManager !== 'undefined' && syncManager.baseUrl) ? syncManager.baseUrl.replace(/\/$/, '') : (typeof window !== 'undefined' ? window.location.origin : '');
        try {
            const res = await fetch(`${base}/api/bins/${encodeURIComponent(binId)}`, {
                method: 'PATCH',
            ...
            const json = await res.json();
            if (json.bin) {
                const local = this.updateBin(binId, json.bin);
                if (typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('bin:updated', { detail: { binId, fill: json.bin.fill } }));
                return local || json.bin;
            }
            return this.updateBin(binId, updates);
        } catch (e) {
            console.warn('updateBinViaAPI failed, updating locally only:', e.message);
            return this.updateBin(binId, updates);
        }
    }
    ...
    updateCollectionWithSensorFill(binId, newFill) {
        ...
        if (c.collectionVerification === 'pending_sensor') {
            const SENSOR_EMPTY_THRESHOLD = 15;
    ...
    addCollection(collection) {
        ...
        if (hasSensor) {
            ...
            collection.collectionVerification = 'pending_sensor';
        } else {
            collection.verifiedBySensor = null;
        ...
        if (typeof window !== 'undefined' && window.dispatchEvent) {
            ...
            // Ensure map bin marker updates to 0% (same as manual "Mark as collected")
